...
class BlazePose(nn.Module):
    ...
    def __init__(self, num_keypoints):
        ...
        self.backbone = timm.create_model("fastvit_t8", pretrained=True, features_only=True, num_classes=0)

        # Get the channel counts from the backbone -> [48, 96, 192, 384]
        encoder_channels = self.backbone.feature_info.channels()

        self.bb1 = nn.Conv2d(encoder_channels[0], 32, 1)  # 56 x 56
        self.bb2 = nn.Conv2d(encoder_channels[1], 64, 1)  # 28 x 28
        self.bb3 = nn.Conv2d(encoder_channels[2], 128, 1) # 14 x 14
        self.bb4 = nn.Conv2d(encoder_channels[3], 192, 1) # 7 x 7
    ...
    def forward(self, x):
        ...
        y1 = self.bb1(features[0]) # 56x56
        y2 = self.bb2(features[1]) # 28x28
        y3 = self.bb3(features[2]) # 14x14
        y4 = self.bb4(features[3]) # 7x7
        # y1 = self.bb2(x)
        # y2 = self.bb3(y1)
        # y3 = self.bb4(y2)
